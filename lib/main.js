...
function forEach( str, options, clbk ) {
	...
	var cb;
	if ( !isString( str ) ) {
		throw new TypeError( format( '1gr3F', str ) );
	}
	opts = {
		'mode': 'grapheme'
	};
	...
	if ( nargs === 2 ) {
		...
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '1gr2V', options ) );
		}
		cb = clbk;
		thisArg = arguments[ 3 ];
	}
	if ( !isFunction( cb ) ) {
		throw new TypeError( format( '1gr2b', cb ) );
	}
	if ( options ) {
		if ( hasOwnProp( options, 'mode' ) ) {
			opts.mode = options.mode;
			if ( !isMode( opts.mode ) ) {
				throw new TypeError( format( '1grE1', 'mode', MODES.join( '", "' ), opts.mode ) );
			}
		}
	}
